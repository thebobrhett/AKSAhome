...
strSQL = "select * from Items where Title='Subway" & WeekdayName(Weekday(now)) & "'"
...
if not objRSNews.eof then
  ...
  if (isnull(objRSNews("PostingDate")) or objRSNews("PostingDate") < Now()) and (isnull(objRSNews("RemovalDate")) or objRSNews("RemovalDate") > Now()) then
    objRSNews.close
    strSQL = "update Items set Test=True where Title='Subway" & WeekdayName(Weekday(now)) & "'"
    objRSNews.open strSQL, objDBNews, adOpenStatic, adLockOptimistic
    strSQL = "update Items set Test=False where Title='SubwayNews'"
    objRSNews.open strSQL, objDBNews, adOpenStatic, adLockOptimistic
  end if
end if
